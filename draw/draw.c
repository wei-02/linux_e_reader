...
static PT_SpeakerOpr g_ptDefaultSpeaker = NULL;
...
{
	int iError;
	g_ptDefaultSpeaker = GetSpeakerOpr("espeak");
	if (!g_ptDefaultSpeaker)
	{
		return -1;
	}

	iError = g_ptDefaultSpeaker->SpeakerInit();
	return iError;
}
...
{
	static char *s_pcTextBuf = NULL;
	int iLen;

	/* 1. 检查当前页面指针是否有效 */
	if (!g_ptCurPage || !g_ptCurPage->pucLcdFirstPosAtFile || !g_ptCurPage->pucLcdNextPageFirstPosAtFile)
	{
		return NULL;
	}

	/* 2. 计算当前页文本字节长度 */
	iLen = g_ptCurPage->pucLcdNextPageFirstPosAtFile - g_ptCurPage->pucLcdFirstPosAtFile;
	...
	/* 4. 分配内存（长度 + 1 用于存放 '\0'） */
	s_pcTextBuf = malloc(iLen + 1);
	if (!s_pcTextBuf)
	{
		DBG_PRINTF("Error: GetCurrentPageContent malloc failed!\n");
		return NULL;
	}

	/* 5. 拷贝数据并封口 */
	memcpy(s_pcTextBuf, g_ptCurPage->pucLcdFirstPosAtFile, iLen);
	s_pcTextBuf[iLen] = '\0';
...
void ReadPage(void)
{
    ...
    if (!g_ptDefaultSpeaker)
    {
        printf("Voice engine not initialized!\n");
        return;
    }

    /* 2. 获取当前页内容 (利用你刚才在 draw.c 增加的函数) */
    pcContent = GetCurrentPageContent();

    if (pcContent)
    {
        printf("Start reading this page...\n");
        /* 3. 先停止之前的朗读，再开始新的朗读 */
        g_ptDefaultSpeaker->StopSpeak();
        g_ptDefaultSpeaker->Speak(g_ptDefaultSpeaker, pcContent);
    }
}
