...
int main(int argc, char **argv)
{
	...
	iError = SelectAndInitSpeaker("espeak");
	if (iError)
	{
		printf("SelectAndInitSpeaker error!\n");
		return -1;
	}
	DBG_PRINTF("%s %s %d\n", __FILE__, __FUNCTION__, __LINE__);
	...
	printf("Enter 'n' to show next page, 'u' to show previous page, 's' to read this page, 'q' to exit: ");
	...
	while (1)
	{
		...
		if (0 == GetInputEvent(&tInputEvent))
		{
			...
			else if (tInputEvent.iVal == INPUT_VALUE_READ)
			{
				ReadPage();			
			}
